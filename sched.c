...
static inline void rqe_insert(struct rq *rq, struct task_struct *p, int flags, char rqe_type)
{
  ...
  if (purpose == 2){
    ...
    rq->rqe_Queue.buffer[rq->rqe_Queue.rqe_insertIdx].dynamicP = p->static_prio;
    ...
    rq->rqe_Queue.buffer[rq->rqe_Queue.rqe_insertIdx].numTasks = rq->nr_running;  
