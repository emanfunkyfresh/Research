...
static int rqe_Get(struct seq_file *m, void *v)
{
  ...
  while ((logEntryLen <= (BLCK_SIZE - BLK_PAD)) && (rq->rqe_Queue.rqe_deleteIdx != stopIndex)) {
    ...
    logEntryLen +=
      sprintf(&logEntry[logEntryLen], "%lld,%s,%ld,%c,%lld,%lld,%ld,%ld,%ld\n",
	      ...
	      rq->rqe_Queue.buffer[rq->rqe_Queue.rqe_deleteIdx].eventTime, /* times, in ns */
	      rq->rqe_Queue.buffer[rq->rqe_Queue.rqe_deleteIdx].vrun,
	      (long long)rq->rqe_Queue.buffer[rq->rqe_Queue.rqe_deleteIdx].normPrio,
	      rq->rqe_Queue.rqe_deleteIdx,
	      stopIndex
	      ); 
